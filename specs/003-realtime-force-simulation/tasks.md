...
- [x] T017 [P] [US1] Add fail-first worker tests for real topology plus step 0, exactly one outstanding Float32 frame, exact buffer-return paint acknowledgement, one D3 tick per global step, contiguous global numbering across initial/control epochs, initial/epoch success terminal snapshots, exact commit-buffer return, painted step-256 `NOT_CONVERGED`, and explicit rejection of batching, dropping, replacement, interpolation, or accepted-command coalescing in `tests/layout-worker.test.js` (Covers: FR-001, FR-003, FR-003a, FR-006, FR-011, FR-015, QR-002, QR-005, SC-009, SC-010)
- [x] T018 [P] [US1] Add fail-first runner tests for `onReady`/`onStep` logical paint receipts, no next-step request before the exact outstanding receipt, duplicate/gapped/wrong-request/malformed frame rejection, hidden active-time accounting hooks, unchanged final Promise settlement, and the `calculateForceLayout()` compatibility plus force `runLayout()` path preserving authoritative assignments/topology/terminal Float32 centers without legacy placement or projection in `tests/layout-runner.test.js` (Covers: FR-003, FR-006a, FR-006b, FR-009, QR-002, QR-005, SC-006, SC-009, SC-013)
- [x] T019 [P] [US1] Add fail-first live-island tests for step-0 construction, exact-next-step acceptance, unique assignments, same-frame tower and spring endpoints, single-leaf/no-relation zero-spring rendering, one spring per active relationship at `y = 0`, force styling/depth flags, current bounds/metadata, no per-step scene/backing-array allocation, required force `terminalFrame` stable input, terminal Float32 center identity, and no stable reconstruction movement in `tests/island.test.js` (Covers: FR-004, FR-005, FR-006a, FR-006b, FR-012, QR-003, QR-005, SC-003, SC-013)
- [x] T020 [P] [US1] Add fail-first local Playwright Chromium journeys tagged `@US1-realtime-force` and loaded with `?testDiagnostics=1` for step-0/status latency, visible copy identifying moving lines as force relationships influencing layout, explicit settled-at-final-step status, at least two distinct moving states, lossless `forceSession.trace()` relationship-oracle validation across ten representative-fixture traces, every-step contiguous paint timestamps/counts, evolving assignments without teleportation, one displayed root, no terminal snap, copied settlement summaries, typed seam rejection, and an interaction epoch that commits or rolls back through the exact seam in `tests/app.spec.js` (Covers: FR-001, FR-002, FR-003, FR-003a, FR-004, FR-005, FR-006b, FR-014, FR-015, SC-001, SC-003, SC-008, SC-009, SC-010, SC-013)

### Implementation for User Story 1

- [x] T021 [P] [US1] Implement `ready` step 0 followed by an exact `painted`-receipt/one-tick loop, one outstanding returned position buffer, contiguous global/epoch/cooling metadata, assignment/convergence metrics, initial/epoch success terminal snapshots with commit-buffer return, and painted step-256 non-convergence publication in `src/layout-worker.js` (Covers: FR-001, FR-003, FR-003a, FR-006, FR-011, FR-015, QR-002, QR-005, SC-009, SC-010)
- [x] T022 [P] [US1] Implement validated all-steps presentation callbacks, exact outstanding-buffer receipts, strict next-global-step enforcement, logical paint completion, and result validation in `src/layout-runner.js` without any gap-tolerant or latest-frame path (Covers: FR-003, FR-009, QR-005, SC-006, SC-009)
- [x] T023 [P] [US1] Implement `createLiveIsland()`/`applyStep()` and force-mode `createIsland({ layoutResult, topology, terminalFrame })` with detached validation, canonical node-index mapping, one occupied `InstancedMesh`, one dynamic `LineSegments` buffer, reusable math/resources, atomic same-frame tower/spring updates, direct terminal-frame Float32 stable transforms, exact order checks, force visual semantics, current bounds, and idempotent disposal in `src/island.js` (Covers: FR-004, FR-005, FR-006a, FR-006b, FR-012, QR-003, SC-003, SC-013)
- [x] T024 [US1] Implement initial and test-seam interaction-epoch stable/live scene transactions, the exact `window.__hexWorldTest.forceSession` submit/wait/commit-outcome/diagnostics API from `contracts/render-contract.md`, `onEpochReady`/`onEpochSettled` provisional presentation, RAF-backed paint receipts, exact terminal equality validation, force topology/`terminalFrame` handoff to stable construction, epoch-specific commit confirmation, rollback on construction/commit failure, and opt-in frame/paint/worker/listener/timer lifecycle diagnostics in `src/main.js` (Covers: FR-001, FR-003, FR-004, FR-006b, FR-011, FR-012, FR-014, FR-015, QR-005, SC-003, SC-009, SC-010, SC-011, SC-013)
- [x] T025 [P] [US1] Add semantic visible calculation progress at `#layout-progress` with separate polite start/terminal status and `aria-live="off"` fields for current global step, epoch, cooling step, assignment stability, convergence streak, maximum step, and terminal reason in `index.html` (Covers: FR-002, FR-003a, FR-008, QR-004, SC-001, SC-009)
- [x] T026 [US1] After T025, style `#layout-progress` and its `[data-force-progress]` fields for clear calculating/terminal reading on desktop and at 360 pixels without obscuring the canvas, relying on text rather than motion or color alone in `src/style.css` (Covers: FR-003a, FR-008, QR-004, SC-009)
- [x] T027 [US1] Wire visible non-live progress updates, explanatory force-relationship text, explicit settled-at-final-step status, start/terminal-only announcements, current/final step counts, maximum-step failure reason, and assignment/convergence diagnostics to worker frames without per-step live announcements in `src/main.js` (Covers: FR-002, FR-003a, FR-006, FR-008, FR-011, QR-004, SC-001, SC-008, SC-009)
- [x] T028 [US1] Implement force terminal-result revalidation and authoritative assignment/topology/Float32 handoff for the fail-first T018 assertions in `src/layout-runner.js` without legacy placement, post-convergence assignment, projection, or snap (Covers: FR-006a, FR-006b, QR-002, SC-013)
- [x] T029 [US1] Run `npm test -- tests/force-layout.test.js tests/layout-worker.test.js tests/layout-runner.test.js tests/island.test.js tests/layout.test.js` and fix the US1 contract slice in `src/force-layout.js`, `src/layout-worker.js`, `src/layout-runner.js`, `src/island.js`, and `src/main.js` (Covers: FR-001, FR-003, FR-004, FR-005, FR-006, FR-006a, FR-006b, QR-002, QR-005, SC-003, SC-009, SC-013)
- [x] T030 [US1] Run the US1 local Chromium scenarios with `npm run test:e2e -- --grep "@US1-realtime-force"`, require zero failures, and record all-frame step/paint/assignment/relationship-oracle traces, epoch transaction results, and terminal identity in `specs/003-realtime-force-simulation/validation/us1.md`; leave the task incomplete on any failed or missing scenario (Covers: FR-001, FR-002, FR-003, FR-003a, FR-004, FR-005, FR-006b, FR-014, FR-015, QR-004a, SC-001, SC-003, SC-009, SC-010, SC-013)
...
- [x] T032 [P] [US2] Implement active-session invalidation before worker termination, stale callback/receipt isolation, supersession and failure settlement, and idempotent release of worker/listener/buffer/deferred/guard ownership in `src/layout-runner.js` (Covers: FR-009, FR-011, FR-012, FR-014, QR-005, SC-006, SC-011)
...
- [x] T034 [P] [US2] Implement interaction-aware matrix composition, current bounds/metadata, semantic selection continuity, atomic validation-before-mutation, retired-handle rejection, and owned-versus-shared GPU disposal in `src/island.js` (Covers: FR-008, FR-009, FR-012, QR-005, SC-003, SC-006, SC-011)
...
- [x] T064 CRITICAL Remove the fixed depth-16 hierarchy rejection while retaining depth-16 verification fixtures in `src/data.js` and `tests/fixtures/hierarchies.js` per Constitution I / QR-001 (contradicts)
- [x] T065 CRITICAL Replace per-step frame, metric, assignment, and live-render allocations with reusable bounded storage in `src/force-layout.js` and `src/island.js`, with allocation regression coverage in `tests/force-layout.test.js` and `tests/island.test.js`, per Constitution III / plan: reusable state (contradicts)
- [x] T066 CRITICAL Add version-2 session, worker, runner, browser, and resource tests and produce complete truthful validation artifacts under `specs/003-realtime-force-simulation/validation/`, leaving no validation claim unsupported by a passing required command, per Constitution V / SC-002 / SC-004 / SC-010 / SC-011 / SC-012 / SC-013 (missing)
- [x] T067 CRITICAL Remove timer-based successful paint acknowledgement from `src/main.js` and retain the exact outstanding frame until an actual logical application paint, with hidden/stalled presentation coverage in `tests/app.spec.js`, per FR-003 / US1/AC2 (contradicts)
- [x] T068 Restore convergence to require three real unchanged assignment epochs, inclusive target-error gates, mobile anchors, and epoch cooling-step minimum/maximum budgets in `src/force-layout.js`, with exact boundary tests in `tests/force-layout.test.js`, per FR-006 / US1/AC3 (contradicts)
- [x] T069 Canonically rank each leaf's reusable assignment candidates by quantized total cost, `q`, and `r` before deferred acceptance in `src/force-layout.js`, with deterministic conflict tests in `tests/force-layout.test.js`, per FR-006a / QR-002 (contradicts)
- [x] T070 Reject semantically invalid release commands without changing accepted watermarks, simulation state, or frame sequence in `src/force-layout.js` and `src/layout-worker.js`, with protocol tests in `tests/force-layout.test.js` and `tests/layout-worker.test.js`, per FR-015 / QR-006 (contradicts)
- [x] T071 Convert all version-2 construction, normalization, cyclic-input, configuration, and environment failures into canonical typed envelopes before session mutation in `src/layout-worker.js` and `src/layout-runner.js`, with category/detail tests, per FR-011 / QR-005 (partial)
- [x] T072 Enforce strict request, epoch, sequence, buffer-length, terminal-frame, receipt, waiter, observer-view, and suppression-race validation in `src/layout-worker.js` and `src/layout-runner.js`, with malformed/stale/racing protocol tests, per QR-005 / SC-009 / SC-010 / SC-011 (partial)
- [x] T073 Add independently owned 60-second visible-active-time guards for forwarded command receipts and explicitly rearm the interaction cooling guard after final release in `src/layout-runner.js`, with hidden-time and cleanup tests, per FR-014 / FR-015 (partial)
- [x] T074 Emit and dispatch the contracted epoch-ready metadata/frame, resample presentation preference for every interaction epoch, and support later-epoch final-only execution in `src/force-layout.js`, `src/layout-worker.js`, and `src/layout-runner.js`, with epoch transaction tests, per FR-014 / FR-015 (partial)
- [x] T075 Centralize the specified `Math.round(value / step) * step` quantization and canonicalize plane/fractional-axial coordinates before inclusive radius validation and transcript storage in `src/hex.js` and `src/force-layout.js`, with tie and radius-boundary tests, per QR-002 / QR-006 (contradicts)
- [x] T076 Replace transferred terminal-buffer wrapper-identity comparison with request/epoch/state ownership and exact shape validation in `src/layout-worker.js`, and add a real transfer round-trip commit test in `tests/layout-worker.test.js`, per FR-014 (contradicts)
- [x] T077 Restrict polite live-region updates to calculation start and terminal status while keeping per-step fields in the `aria-live="off"` progress region in `src/main.js` and `index.html`, with browser semantic assertions in `tests/app.spec.js`, per FR-003a / QR-004 (contradicts)
- [x] T078 Keep the layout selector, generator fields, submit action, and reset-camera control pointer/touch/keyboard operable during calculation by removing the busy-form pointer block in `src/style.css` and testing unmodified hit targets in `tests/app.spec.js`, per FR-008 / US2/AC1 (contradicts)
- [x] T079 Make initial and interaction-epoch scene commits detached, confirmation-gated, and rollback-safe so the previous stable island remains authoritative on construction or commit failure in `src/main.js`, with initial/epoch failure tests in `tests/app.spec.js`, per FR-011 / FR-014 (contradicts)
- [x] T080 Reuse live-island update storage, maintain current raycast bounds, request stationary-pointer hover reevaluation, and restore selection by stable entity ID in `src/island.js` and `src/main.js`, with live interaction tests in `tests/island.test.js`, per FR-008 / Constitution III (partial)
- [x] T081 Preserve validated cycle members/path and missing platform/capability details in user-visible failure messages in `src/main.js`, with cyclic and unsupported browser journeys in `tests/app.spec.js`, per FR-011 (partial)
- [x] T082 Implement the exact two-warmup/ten-run browser-clock measurement protocol, complete profile/fixture/motion matrix, specified paint endpoints, contiguous cadence trace, reset-camera input cohorts, fixed limits, and 300-interval post-commit sample in `tests/layout.benchmark.spec.js`, per QR-003 / SC-001 / SC-005 (contradicts)
- [x] T083 Add the artifact-specified `@US1-realtime-force`, `@US2-usable-lifecycle`, and `@US3-deterministic-reduced-motion` diagnostic browser journeys, including exact frame/spring traces, hidden/pagehide lifecycle, epoch commit/rollback, reduced-motion equality, gesture negatives, and all failure categories in `tests/app.spec.js`, per US1-US3 acceptance / SC-003 / SC-006 / SC-007 / SC-008 / SC-009 (missing)
- [x] T084 Instrument real worker, idle tick/message/timer, listener, scene-root, and GPU lifecycle counters in `src/main.js` and validate repeated settle/rebuild/cancel/switch/pagehide cycles in `tests/resource-profile.spec.js`, recording raw evidence in `specs/003-realtime-force-simulation/validation/resources.md`, per SC-011 / Constitution III (contradicts)
- [x] T085 Document hidden-frame pause/resume, initial and interaction-epoch rollback, global-step continuity, release cooling budgets, command rejection states, teardown ownership, and the complete verification command set in `README.md` per T060 (partial)
- [x] T086 Consolidate synchronous and worker force calculation on the approved version-2 session in `src/force-layout.js`, retaining only a justified thin request adapter if a current caller requires compatibility and updating tests accordingly, per Constitution V / plan: single deterministic session (unrequested)
- [x] T087 Remove the extra visual projects from this feature's blocking/default validation command or document and configure them explicitly as non-blocking in `playwright.config.js` and `package.json` per QR-004a (unrequested)